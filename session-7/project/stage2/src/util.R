...
compare_country <- function(data, isQuarto = F){
  ele <- div(
    h2(data$name_long),
    p(tags$b("Continent: "), data$continent),
    p(tags$b("Region: "), data$region_un),
    p(tags$b("Subregion: "), data$subregion),
    
    if(isQuarto) {
      img(src = data$flag, viewBox = "0 0 600 300", width = 600, height = 300)
    } else {
      img(src = data$flag)
    }
...
generate_compare_plot <- function(data, plot_label, plot_id, isGGplot = F) {
  
  chart_colours <- RColorBrewer::brewer.pal(n = max(3, nrow(data)),
                                            name = "Paired")   
  ...
  if(isGGplot) {
    ...
    #Get required number of colors
    chart_colours <- chart_colours[1:nrow(data)]
    
    # Get plot
    fig <- ggplot(data, aes(x=name_long, y= .data[[plot_id]])) + 
      geom_bar(stat = "identity", fill = chart_colours) +
      xlab("Country") + 
      ylab(plot_label)
