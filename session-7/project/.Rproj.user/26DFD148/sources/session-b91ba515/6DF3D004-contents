...
table_page_server <- function(id, filtered_data, filter_inputs) {
  ...
  moduleServer(
    ...
    function(input, output, session) {
      ...
      download_server("download", filtered_data)
      # Summary outputs ---------------------------------------------------------
      ...
      output$filter_summary <- renderUI({
        tagList(
          ...
          tags$ul(
            tags$li(sprintf("Filters: %s, %s, (%s)", filter_inputs()$region, filter_inputs()$subregion, 
                            paste(filter_inputs()$country, sep = ",", collapse = ","))),
